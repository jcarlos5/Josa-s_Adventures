...
namespace Invector.vCharacterController
{
    ...
    public class vShooterMeleeInput : vMeleeCombatInput, vIShooterIKController, PlayerController.vILockCamera
    {
        ...
        public virtual void LoadIKAdjust(string weaponCategory)
        {
            if (shooterManager)
            {
        ...
        protected virtual bool IsIgnoreIK
        {
            get
            {
        ...
        public virtual void AimInput()
        {
            ...
            if (shooterManager.hipfireShot)
            {
                ...
                if (_aimTimming > 0 && (!shooterManager.isShooting&&!shooterManager.isShootingEmptyClip))
                {
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            Debug.Log(weaponSide);
            var _ray = new Ray(cc._capsuleCollider.bounds.center + transform.up * shooterManager.blockAimOffsetY + transform.right * shooterManager.blockAimOffsetX * weaponSide - transform.forward * cc._capsuleCollider.radius, cameraMain.transform.forward);
            Gizmos.DrawRay(_ray.origin, _ray.direction * shooterManager.minDistanceToAim);
            var color = Gizmos.color;
            color = aimConditions ? Color.green : Color.red;
            color.a = 1f;
            Gizmos.color = color;
            Gizmos.DrawSphere(aimAngleReference.transform.position, .05f);
            Gizmos.DrawLine(aimAngleReference.transform.position, _ray.origin);
        ...
        protected virtual void RotateAimArm(bool isUsingLeftHand = false)
        {
            ...
            if (CurrentActiveWeapon && armAlignmentWeight > 0.01f && CurrentActiveWeapon.alignRightUpperArmToAim)
            {
                ...
                {
                    upperArmRotationAlignment = shooterManager.isShooting && armAlignmentWeight>.9f ? upperArmRotation : rot;
                }
        ...
        protected virtual void RotateAimHand(bool isUsingLeftHand = false)
        {
            ...
            if (CurrentActiveWeapon && armAlignmentWeight > 0.01f && aimConditions && CurrentActiveWeapon.alignRightHandToAim)
            {
                ...
                if ((!float.IsNaN(rot.x) && !float.IsNaN(rot.y) && !float.IsNaN(rot.z)))
                {
                    handRotationAlignment = shooterManager.isShooting && armAlignmentWeight > .9f ? handRotation : rot;
                }
