...
namespace Invector
{
    public static class vExtensions
    {
        ...
        /// <summary>
        /// Get angle Y of two Transforms
        /// </summary>
        /// <param name="self"></param>
        /// <param name="target"></param>
        /// <returns></returns>
        public static float AngleYBetweenTransform(this Transform self,Transform target)
        {           
            var selfForward = self.forward;
            selfForward.y = 0;
            var selfEuler = Quaternion.LookRotation(selfForward.normalized).eulerAngles;

            var targetForward = target.forward;
            targetForward.y = 0;
            var targetEuler = Quaternion.LookRotation(targetForward.normalized).eulerAngles;

            return (targetEuler- selfEuler).NormalizeAngle().y ; 
        }
        /// <summary>
